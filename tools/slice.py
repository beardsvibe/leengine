...
void {{prefix}}_load({{prefix}}_impl_t * s, scene_load_font_t load_font)
{
	...
	memset(&s->{{name}}, 0, sizeof(s->{{name}}));
	s->{{name}}.entity         = &s->{{entity_name}};
	s->{{name}}.original_text  = "{{text}}";
	s->{{name}}.text           = s->{{name}}.original_text;
...
class Slicer:
	...
	def _export(self, plane, parent_name = "", parent_visible = True):
		...
		if any([ign in name for ign in self.ignored_layers]): # first check if it's ignored
			return
		...
		if hasattr(plane, "layers"):
			...
			# in case on 9slice will contain texture name
			slice9_texture_name = None
			slice9_name = None

			cmds = ["!set_9slice", "!use_9slice"]
			if any([cmd in raw_name for cmd in cmds]):
				...
				if "!set_9slice" in raw_name:
					# save 9slice
					if len(m) != 5:
						raise ValueError("invalid command argument in name '%s'/'%s', expected 5 args" % (name, raw_name))
					...
					self.slice9.append({
						"index": len(self.slice9),
						"name": "slice9_" + m[0],
						"u1": self.images_scale_factor * float(m[1]) / float(img_w),
						"v1": self.images_scale_factor * float(m[2]) / float(img_h),
						"u2": self.images_scale_factor * float(m[3]) / float(img_w),
						"v2": self.images_scale_factor * float(m[4]) / float(img_h),
						"texture_name": texture_name
					})

					return
				elif "!use_9slice" in raw_name:
					...
					for v in self.slice9:
						if v.get("name") == "slice9_" + m[0]:
							slice9_name = v.get("name")
							slice9_texture_name = v.get("texture_name")
							break

					if not slice9_texture_name:
						raise ValueError("can't find 9slice with name '%s' in layer '%s'/'%s'" % (m[0], name, raw_name))

			elif any([ch in raw_name for ch in ["[", "]", "!", ","]]):
				# check for invalid commands
				raise ValueError("unknown command sequence in name '%s'" % (name))
			...
			if plane.text_data:
				...
				try:
					...
					# reuse 9slice texture
					if slice9_texture_name:
						texture_name = slice9_texture_name
					else:
						img = plane.as_PIL()
						img = img.resize([int(self.images_scale_factor * s) for s in img.size])
						texture_name = self._cache_images(filename, img)
